...
def complete_action(db: Session, user: User, review_id: str, new_status: str) -> dict:
    # Validate status against the enum (#9)
    try:
        validated_status = ActionStatus(new_status)
    except ValueError:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail=f"Invalid status '{new_status}'. Must be one of: completed, skipped",
        )
    ...
    review.action_status = validated_status
...
def _build_weekly_snapshot(db: Session, user: User, week_start: date, week_end: date) -> dict:
    # Reuse shared aggregation logic (#10)
    ...
    return {
        ...
        "monthly_expenses": agg["monthly_expenses"],
        "cash_flow": agg["cash_flow"],
        "credit_utilization_pct": agg["credit_utilization_pct"],
        "savings_balance": agg["savings_balance"],
...
def _generate_action(db: Session, user: User, snapshot: dict, week_start: date) -> dict:
    ...
    # --- Credit utilization rules (use elif to avoid overlapping candidates, #3) ---
    if cards:
    ...
    # --- Emergency fund rules (use elif to avoid overlapping candidates, #4) ---
    ...
    # Anti-repeat: check last 2 reviews (#1 — fixed dead code + proper fallback)
    recent_reviews = (
        db.query(WeeklyReview)
        .filter(WeeklyReview.user_id == user.id, WeeklyReview.week_start < week_start)
        .order_by(WeeklyReview.week_start.desc())
    ...
    if len(recent_types) == 2 and all(t == winner["type"] for t in recent_types):
        for score, candidate in candidates[1:]:
            if candidate["type"] != winner["type"] and score >= 30:
                return candidate
        # All alternatives exhausted or scored too low — return fallback
